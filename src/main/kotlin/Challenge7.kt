...
class Challenge7(private val file: File) {
...
//        val plaintext = aesecb.decrypt(AdvancedUtils.readAndDecodeFile(file).dropLast(12).toByteArray(StandardCharsets.UTF_8), CodecSupport.toBytes(key))
        val plaintext = aesecb.decrypt(AdvancedUtils.readAndDecodeFile(file).toByteArray(StandardCharsets.UTF_8), CodecSupport.toBytes(key))
...
fun decryptAES128ECB(base64Ciphertext: String, key: String): String {
    val aesecb = AESECB()
    val plaintext = aesecb.decrypt(Base64.decode(base64Ciphertext), CodecSupport.toBytes(key))
...
//    println(Challenge7(File("src/main/resources/Challenge7.txt")).decryptAES128ECB("YELLOW SUBMARINE"))
    ...
    val text = "CRIwqt4+szDbqkNY+I0qbDe3LQz0wiw0SuxBQtAM5TDdMbjCMD/venUDW9BL" +
    ...
    println(decryptAES128ECB(text, key))
}