class Challenge3(val encodedString: String) {
    ...
    private fun scoreString(string: String) = string.map { character -> letterStats[character] ?: 0.0 }.sum()

    private fun xorWithCharacter(string: String, character: Char): String {
        val code = character.toString().repeat(string.length)
        return Utils.xorString(string, code)
    }

    class BruteForceResult(val character: Char, val string: String, val score: Double)

    fun findBestMatch() {
        val bestMatch = Utils.hexCharacters.map { character ->
